...
pub fn render(f: &mut Frame, area: Rect, input: &str, _cursor: usize, suggestions: &[String]) {
    ...
    let input_area = Rect::new(
        area.x,
        area.y + if suggestions.is_empty() { 0 } else { 1 },
        area.width,
        1,
    );
    let line = Line::from(vec![Span::styled(": ", theme::accent()), Span::raw(input)]);
