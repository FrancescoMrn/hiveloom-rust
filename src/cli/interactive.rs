...
async fn handle_key(app: &mut App, code: KeyCode, mods: KeyModifiers) -> anyhow::Result<bool> {
    ...
    match &mut app.screen {
        ...
        Screen::Submenu {
            ...
            KeyCode::Tab => {
                *focus = if *focus == PanelFocus::Actions {
                    PanelFocus::Context
                } else {
                    PanelFocus::Actions
                }
                    ...
                    Ok(resp) => {
                        *conversation_id = Some(resp.conversation_id);
                        messages.push(ChatMessage {
                            ...
                            capabilities: resp.capabilities_used,
                        });
                    }
                    Err(e) => {
                        messages.push(ChatMessage {
        ...
        Screen::CmdBar {
            ...
            KeyCode::Backspace => {
                ...
                *suggestions = filter_cmd(&app.cmd_registry, input);
            }
            KeyCode::Char(c) if !mods.contains(KeyModifiers::CONTROL) => {
                input.insert(*cursor, c);
                *cursor += 1;
    ...
    match cat {
        ...
        Category::Chat => {
            if let Some(agent) = app.overview.agents.first() {
                ...
                app.status_msg = Some((
                    "No agents. Create one first via Agents > Create.".into(),
                    false,
                ));
...
fn category_actions(cat: Category) -> Vec<String> {
    match cat {
        Category::Agents => vec![
            "Create new agent".into(),
            "Add skill".into(),
            "Export".into(),
        ],
        Category::Credentials => vec![
            "Set new credential".into(),
            "Rotate".into(),
            "Remove".into(),
        ],
    ...
    if focus == PanelFocus::Context {
        ...
        match cat {
            Category::Agents => match aidx {
                ...
                _ => {
                    app.status_msg = Some((
                        "Use the context panel to select an agent first.".into(),
                        false,
                    ));
                }
            },
            ...
            Category::Mcp => {
                if aidx == 0 {
                    let agent_names: Vec<String> =
                        app.overview.agents.iter().map(|a| a.name.clone()).collect();
                    if agent_names.is_empty() {
                        app.status_msg = Some(("Create an agent first.".into(), false));
                    } else {
                        app.screen = Screen::Form {
                            parent: cat,
                            title: "Create MCP Identity".into(),
                            form: FormState::new(vec![
...
async fn handle_popup_action(
    ...
    match (cat, action) {
        (Category::Agents, "Chat") => {
            app.screen = Screen::ChatMode {
                agent_name: item_name.into(),
                agent_id: item_id.into(),
                conversation_id: None,
                messages: vec![],
                input: String::new(),
                cursor: 0,
        ...
        (Category::Agents, "Delete") => {
            ...
            match app
                .client
                .delete(&format!("/api/tenants/{}/agents/{}", tid, item_id))
                .await
            {
        ...
        (Category::Agents, "Add Skill") => {
            app.screen = Screen::Form {
                parent: cat,
                title: format!("Add Skill to {}", item_name),
                form: FormState::new(vec![
                    FormField::text("Skill Name", "product-faq"),
                    FormField::text("Description", "Knowledge base"),
                    FormField::text("File Path", "skills/faq.md"),
        ...
        (Category::Credentials, "Remove") => {
            ...
            match app
                .client
                .delete(&format!("/api/tenants/{}/credentials/{}", tid, item_id))
                .await
            {
                Ok(()) => {
                    app.status_msg = Some((format!("Removed credential '{}'", item_name), true))
                }
                Err(e) => app.status_msg = Some((format!("Failed: {}", e), false)),
            }
            app.refresh().await;
        ...
        (Category::Mcp, "Reissue Code") => {
            run_cli_command(
                app,
                &format!(
                    "mcp-identity reissue-setup-code {} --tenant default",
                    item_id
                ),
            );
            enter_category(app, cat);
        ...
        (Category::Mcp, "Revoke") => {
            run_cli_command(
                app,
                &format!("mcp-identity revoke {} --tenant default", item_id),
            );
            app.refresh().await;
            enter_category(app, cat);
        }
...
async fn submit_form(app: &mut App, kind: FormKind, parent: Category) -> anyhow::Result<()> {
    ...
    match kind {
        FormKind::CreateAgent => {
            ...
            match app
                .client
                .post::<_, serde_json::Value>(&format!("/api/tenants/{}/agents", tid), &body)
                .await
            {
                Ok(_) => {
                    app.status_msg = Some((
                        format!("{} Agent '{}' created", theme::ICON_CHECK, name),
                        true,
                    ));
                    app.refresh().await;
                    enter_category(app, parent);
                }
                Err(e) => {
                    if let Screen::Form { form, .. } = &mut app.screen {
        ...
        FormKind::SetCredential => {
            ...
            if secret.is_empty() {
                if let Screen::Form { form, .. } = &mut app.screen {
                    form.error = Some("API key is required".into());
                }
                return Ok(());
            }
            ...
            match app
                .client
                .post::<_, serde_json::Value>(&format!("/api/tenants/{}/credentials", tid), &body)
                .await
            {
                Ok(_) => {
                    app.status_msg = Some((
                        format!("{} Credential '{}' stored", theme::ICON_CHECK, name),
                        true,
                    ));
                    app.refresh().await;
                    enter_category(app, parent);
        ...
        FormKind::CreateMcpIdentity => {
            ...
            match app
                ...
                .post::<_, serde_json::Value>(
                    &format!("/api/tenants/{}/mcp-identities", tid),
                    &body,
                )
                .await
            {
        ...
        FormKind::AddSkill { agent_id } => {
            ...
            if name.is_empty() || path.is_empty() {
                if let Screen::Form { form, .. } = &mut app.screen {
                    form.error = Some("Name and file path are required".into());
                }
                return Ok(());
            }
            let content = match std::fs::read_to_string(path) {
                ...
                Err(e) => {
                    if let Screen::Form { form, .. } = &mut app.screen {
                        form.error = Some(format!("Cannot read file: {}", e));
                    }
                    return Ok(());
                }
            };
            ...
            match app
                .client
                .post::<_, serde_json::Value>(
                    &format!("/api/tenants/{}/agents/{}/capabilities", tid, agent_id),
                    &body,
                )
                .await
            {
            ...
            if !app.overview.credentials.is_empty() {
                ...
                } = &mut app.screen
                {
                    *info_lines = vec![format!(
                        ...
                        theme::ICON_CHECK,
                        app.overview.credentials[0].name
                    )];
                    *form = None;
                }
            } else {
                ...
                {
                    *form = Some(FormState::new(vec![FormField::masked(
                        "API Key",
                        "sk-ant-...",
                    )]));
        ...
        1 => {
            ...
            if let Some(ref f) = form_snapshot {
                ...
                if !key.is_empty() {
                    ...
                    let body =
                        serde_json::json!({ "name": cred_name, "kind": "static", "value": key });
                    match app
                        .client
                        .post::<_, serde_json::Value>(
                            &format!("/api/tenants/{}/credentials", tid),
                            &body,
                        )
                        .await
                    {
                        Ok(_) => {}
                        Err(e) => {
                            if let Screen::Wizard { form: Some(f), .. } = &mut app.screen {
                    ...
                    .await
                {
                    ...
                    if let Screen::Wizard { info_lines, .. } = &mut app.screen {
                        *info_lines = vec![
                            ...
                            format!(
                                "  URL:   {}/mcp/{}/{}",
                                app.overview.endpoint, tid, agent.name
                            ),
                            format!("  Code:  {}", code),
                            String::new(),
                ...
                if !msg.is_empty() {
                    ...
                    if let Some(agent) = app.overview.agents.first() {
                        ...
                        match app
                            .client
                            .post::<_, ChatResponse>(
                                &format!("/api/tenants/{}/agents/{}/chat", tid, agent.id),
                                &body,
                            )
                            .await
                        {
                            Ok(resp) => {
                                ...
                                } = &mut app.screen
                                {
                                    *info_lines = vec![
                                        ...
                                        String::new(),
                                        format!(
                                            "{} Setup complete! Press Enter to finish.",
                                            theme::ICON_CHECK
                                        ),
                                    ];
                                    *form = None;
...
fn filter_cmd(registry: &[String], query: &str) -> Vec<String> {
    ...
    registry
        .iter()
        .filter(|e| e.to_lowercase().starts_with(&q) || e.to_lowercase().contains(&q))
        .take(5)
        .cloned()
        .collect()
}
...
async fn fetch_overview(client: &ApiClient) -> Overview {
    ...
    let agents: Vec<AgentInfo> = client
        ...
        .await
        .unwrap_or_default();
    let credentials: Vec<CredentialInfo> = client
        .get("/api/tenants/default/credentials")
        .await
        .unwrap_or_default();
    let mcp_identities: Vec<McpInfo> = client
        .get("/api/tenants/default/mcp-identities")
        .await
        .unwrap_or_default();
    Overview {
        endpoint,
...
fn render(f: &mut Frame, app: &App) {
    ...
    let breadcrumb = match &app.screen {
        ...
        Screen::Form { parent, title, .. } => Some(Box::leak(
            format!("{} {} {}", parent.label(), theme::ICON_ARROW, title).into_boxed_str(),
        ) as &str),
        Screen::ChatMode { agent_name, .. } => Some(Box::leak(
            format!("Chat {} {}", theme::ICON_ARROW, agent_name).into_boxed_str(),
        ) as &str),
        Screen::Wizard { step, .. } => Some(Box::leak(
            format!(
                "Setup {} {}",
                theme::ICON_ARROW,
                WIZARD_TITLES.get(*step).unwrap_or(&"")
            )
            .into_boxed_str(),
        ) as &str),
        Screen::CmdBar { .. } => None,
    ...
    match &app.screen {
        ...
        Screen::CmdBar {
            ...
            let cmd_area = Rect::new(
                chunks[1].x,
                chunks[1].bottom().saturating_sub(2),
                chunks[1].width,
                2,
            );
...
fn render_main_menu(f: &mut Frame, area: Rect, selected: usize) {
    let items: Vec<MenuItem> = Category::all()
        .iter()
        .map(|c| MenuItem {
            label: c.label().to_string(),
            description: c.description().to_string(),
            badge: if *c == Category::Setup {
                Some(theme::ICON_ARROW.to_string())
            } else {
                None
            },
        })
...
fn render_submenu(
    ...
    // Left: actions
    let action_items: Vec<MenuItem> = actions
        .iter()
        .map(|a| MenuItem {
            label: a.clone(),
...
fn render_wizard(
    ...
    if let Some(fs) = form {
        ...
        let form_area = Rect::new(
            content_area.x,
            content_area.y + info_height + 1,
            content_area.width,
            content_area.height.saturating_sub(info_height + 1),
        );
        form::render(f, form_area, fs);
    } else {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            "  Enter: continue   Tab: skip",
            theme::dim(),
        )));
