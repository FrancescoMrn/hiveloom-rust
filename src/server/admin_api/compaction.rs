...
pub async fn list_compaction_events(
    ...
    let tenant_id = match Uuid::parse_str(&tid) {
        Ok(id) => id,
        Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "invalid tenant_id"}))).into_response(),
    };

    let store = match state.open_tenant_store(&tenant_id) {
        Ok(s) => s,
        Err(_) => return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "tenant not found"}))).into_response(),
    };

    let agent_id = query.agent_id.and_then(|s| Uuid::parse_str(&s).ok());
    let limit = query.limit.unwrap_or(50);

    match CompactionEvent::list(store.conn(), tenant_id, agent_id, query.since.as_deref(), limit) {
...
pub async fn get_compaction_config(
    ...
    let agent_id = match Uuid::parse_str(&aid) {
        Ok(id) => id,
        Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "invalid agent_id"}))).into_response(),
    };
    ...
    match config::resolve_config(store.conn(), tenant_id, agent_id) {
        Ok(resolved) => Json(serde_json::to_value(&CompactionConfigResponse { config: resolved }).unwrap()).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response(),
    }
}
    ...
    if body.reset {
        ...
        match CompactionConfig::get_for_agent(conn, tenant_id, agent_id) {
            ...
            Ok(None) => {
                ...
                let max_summary = body.max_summary_fraction_pct.unwrap_or(config::DEFAULT_MAX_SUMMARY_FRACTION_PCT);
                let protected = body.protected_turn_count.unwrap_or(config::DEFAULT_PROTECTED_TURN_COUNT);
                let indicator = body.show_indicator.unwrap_or(config::DEFAULT_SHOW_INDICATOR);

                if let Err(e) = CompactionConfig::create(
                    conn, tenant_id, Some(agent_id), threshold, max_summary, protected, indicator,
                ) {
                    return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": e.to_string()}))).into_response();
                }
            }
